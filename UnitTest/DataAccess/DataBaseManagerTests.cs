...
namespace UnitTest.DataAccess
{
    public class DataBaseManagerTests
    {
        ...
        public async Task QueryAsync_ShouldReturnExpectedResults()
        {
            ...
            _dbConnectionMock.SetupDapperAsync(db => db.QueryAsync<Category>(It.IsAny<string>(), null, null, null, null))
        ...
        public async Task QuerySingleOrDefaultAsync_ShouldReturnExpectedResult()
        {
            ...
            _dbConnectionMock.SetupDapperAsync(db => db.QuerySingleOrDefaultAsync<Category>(query, new { CategoryId = categoryId }, null, null, null))
                .ReturnsAsync(expectedCategory);

            // When
            var result = await _dataBaseManager.QuerySingleOrDefaultAsync<Category>(query, new { CategoryId = categoryId });
